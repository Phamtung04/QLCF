...
const HDFillterMenu: React.FC<Props> = ({
    ...
    const handleBranchChange = (event: any, newValue: Branch | null) => {
        setSelectedBranch(newValue ? newValue.id : null);
        setSelectedTable(null); // Clear selected table when branch is changed
    };

    const handleTableChange = (event: any, newValue: Table | null) => {
        setSelectedTable(newValue ? newValue.id : null);
    };
    ...
    return (
        <Box>
            <Grid container spacing={1.5} direction={{ xs: 'column', sm: 'row' }}>
                ...
                <Grid item xs={12} sm={6}>
                    <Autocomplete
                        ...
                        renderInput={(params) => <TextField {...params} label="Phòng họp" />}
            ...
            {/* Dialog for closing the website */}
            <Snackbar
                open={dialogOpen}
